...
describe('OAuthClientCredentialsHandler', () => {
  test('beforeFetchRequestAsync throws with invalid access token', () => {
    // Arrange
    const invalidAccessToken: GetAccessTokenResponse = {
      access_token: '',
      scope: '',
      refresh_token: '',
      expires_in: Date.now(),
    ...
    // Act
    const oauthClientCustomTokenCredentialsHandler =
  ...
  test('beforeFetchRequestAsync returns correct regional domain for valid access token', async () => {
    // Arrange
    ...
    const beforeRequestResult: BeforeFetchResult = await oauthClientCustomTokenCredentialsHandler.beforeFetchRequestAsync(
      url,
      request
    );
