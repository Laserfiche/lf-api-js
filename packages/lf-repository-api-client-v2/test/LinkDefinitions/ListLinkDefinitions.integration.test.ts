...
describe('Link Definitions Integration Tests', () => {
  ...
  test('Get Link Definitions Simple Paging', async () => {
    ...
    const response = await _RepositoryApiClient.linkDefinitionsClient.listLinkDefinitions({ repositoryId, prefer });
    ...
    const response2 = await _RepositoryApiClient.linkDefinitionsClient.listLinkDefinitionsNextLink({
      nextLink,
      maxPageSize,
    });
