...
describe('Template Definitions Integration Tests', () => {
  test('Get Template Definition', async () => {
    ...
    const templateInfo: WTemplateInfo = result.value[0];
  ...
  test('Get Template Definition Fields', async () => {
    ...
    const firstTemplateDefinition = templateDefinitionResponse.value[0];
    expect(firstTemplateDefinition).not.toBeNull();
    const result: ODataValueContextOfIListOfTemplateFieldInfo =
  ...
  test('Get Template Definition Fields Simple Paging', async () => {
    ...
    const response = await _RepositoryApiClient.templateDefinitionsClient.getTemplateDefinitions({ repoId: repositoryId, prefer });
    if (!response.value) {
      throw new Error('response.value is undefined');
    }
    ...
    const response2 = await _RepositoryApiClient.templateDefinitionsClient.getTemplateDefinitionsNextLink({
      nextLink,
      maxPageSize,
    });
    if (!response2.value) {
  ...
  test('Get Template Field Definition Fields Simple Paging', async () => {
    ...
    expect(nextLink).not.toBeNull();
    expect(response.value.length).toBeLessThanOrEqual(maxPageSize);
    const response2 = await _RepositoryApiClient.templateDefinitionsClient.getTemplateFieldDefinitionsNextLink({
      nextLink,
      maxPageSize,
    });
    if (!response2.value) {
  ...
  test('Get Template Definition Fields by Id', async () => {
    ...
    const firstTemplateDefinition = templateDefinitionResponse.value[0];
    expect(firstTemplateDefinition).not.toBeNull();
    const result = await _RepositoryApiClient.templateDefinitionsClient.getTemplateDefinitionById({
      repoId: repositoryId,
      templateId: firstTemplateDefinition.id ?? -1,
    });
    expect(result).not.toBeNull();
